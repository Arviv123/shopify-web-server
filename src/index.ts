...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  try {
    switch (request.params.name) {
      ...
      case "get_product": {
        const { productId } = request.params.arguments as any;
        try {
      ...
      case "create_order": {
        const { lineItems, customer, shippingAddress } = request.params.arguments as any;
        try {
      ...
      case "list_orders": {
        const { limit = 10, status } = request.params.arguments as any;
        try {
      ...
      case "compare_products": {
        const { searchTerm } = request.params.arguments as any;
        try {
      ...
      case "search_by_vendor": {
        const { vendor, limit = 20 } = request.params.arguments as any;
        try {
      ...
      case "products_by_price_range": {
        const { minPrice, maxPrice, limit = 20 } = request.params.arguments as any;
        try {
      ...
      // ===== FLIGHT API TOOLS =====
      case "search_flights": {
        const searchParams = request.params.arguments as any;
        try {
      ...
      case "get_flight_details": {
        const { flightId } = request.params.arguments as any;
        try {
      ...
      case "get_popular_destinations": {
        const { origin = "TLV", limit = 10 } = request.params.arguments as any;
        try {
      ...
      case "book_flight": {
        const { flightId, passenger } = request.params.arguments as any;
        try {
